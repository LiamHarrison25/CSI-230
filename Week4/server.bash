...
#Filename variable for wg.config
fileName="${wg0}wg0.conf"

echo "${fileName}"

#check to see if wg.config exists already. 
if [[ -f "${fileName}" ]]
then
	echo "Should the wg.config file be overwritten? Type Y for yes, N for no"
	read userInput
	
	if [[ "${userInput}" == "N" || "${userInput}" == "" ]]
	then
		echo "Exiting Program. Will not overwrite the wg.config file"
	
	elif [[ "${userInput}" == "Y" ]]
	then
		echo "Overwritting the wg.config file"
	else
